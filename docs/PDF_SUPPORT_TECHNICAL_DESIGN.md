...
| `react-native-pdf-jsi` | Convert PDF pages to images (fallback for scanned PDFs) | Yes |
| `@config-plugins/react-native-blob-util` | Required by react-native-pdf-jsi | Yes |
...
| File | Purpose |
|------|---------|
| `lib/parsing/pdfParser.ts` | PDF text extraction and page-to-image conversion |
| `lib/parsing/pdfTypes.ts` | TypeScript types for PDF processing |
...
async function extractTextFromPDFViaOCR(
  ...
  for (let page = 1; page <= pageCount; page++) {
    ...
    const imageUri = await PdfJsi.exportPage(uri, page, {
      format: 'jpeg',
      quality: 0.8,
    });

    // Run OCR on the image (reuse existing function)
    const pageText = await extractTextFromImage(imageUri, `${fileIdentifier} - Page ${page}`);
...
- Install `react-native-pdf-jsi` for page-to-image conversion
- Implement `extractTextFromPDFViaOCR()` fallback
- Auto-detect when to use OCR (text threshold)
...
- Update README with PDF support documentation
