 ...
 * - Convenience methods (postJSON, getJSON)
 */

import { fetchWithRetry, postJSON, getJSON } from '../../../lib/http/httpClient';
...
describe('HTTP Client', () => {
  ...
  describe('postJSON', () => {
    it('should send POST request with JSON body', async () => {
      ...
      const result = await postJSON('https://api.example.com/test', data);
    ...
    it('should throw NetworkRequestError on parse failure', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(NetworkRequestError);
        expect((error as NetworkRequestError).type).toBe('parse');
      }
    });
    ...
    it('should handle complex JSON data', async () => {
      ...
      await postJSON('https://api.example.com/test', complexData);
  ...
  describe('getJSON', () => {
    it('should send GET request and parse JSON response', async () => {
      const mockResponse = new Response('{"data": "test"}', { status: 200 });
      (global.fetch as jest.Mock).mockResolvedValueOnce(mockResponse);

      const result = await getJSON('https://api.example.com/test');
    ...
    it('should merge custom headers', async () => {
      ...
      await getJSON('https://api.example.com/test', {
        headers: {
    ...
    it('should throw NetworkRequestError on parse failure', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(NetworkRequestError);
        expect((error as NetworkRequestError).type).toBe('parse');
        expect((error as NetworkRequestError).message).toContain('parse JSON');
      }
    });
    ...
    it('should handle array JSON response', async () => {
      const mockResponse = new Response('[1, 2, 3]', { status: 200 });
      (global.fetch as jest.Mock).mockResolvedValueOnce(mockResponse);

      const result = await getJSON<number[]>('https://api.example.com/test');
