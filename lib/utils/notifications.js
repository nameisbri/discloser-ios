...
exports.parseReminderDate = parseReminderDate;
exports.getNotificationDate = getNotificationDate;
...
function parseReminderDate(dateString) {
    ...
    if (!dateRegex.test(dateString)) {
        throw new Error("Invalid date format: \"".concat(dateString, "\". Expected YYYY-MM-DD format."));
    }
    // Parse components
    ...
    if (isNaN(year) || isNaN(month) || isNaN(day)) {
        throw new Error("Invalid date components in: \"".concat(dateString, "\""));
    }
    if (month < 1 || month > 12) {
        throw new Error("Invalid month: ".concat(month, ". Must be between 1 and 12."));
    }
    if (day < 1 || day > 31) {
        throw new Error("Invalid day: ".concat(day, ". Must be between 1 and 31."));
    }
    // Create Date at 9:00 AM local time
    // Note: JavaScript months are 0-indexed (January = 0)
    var date = new Date(year, month - 1, day, 9, 0, 0, 0);
    // Validate the date is actually valid (e.g., not Feb 31)
    if (date.getFullYear() !== year ||
        date.getMonth() !== month - 1 ||
        date.getDate() !== day) {
        throw new Error("Invalid date: \"".concat(dateString, "\" does not represent a valid calendar date."));
    }
    return date;
}
/**
 ...
 */
function getNotificationDate(input) {
    ...
    throw new Error("Invalid input type: expected string or Date, got ".concat(typeof input));
}
/**
...
function isValidFutureDate(date) {
    ...
    var oneMinuteFromNow = Date.now() + 60 * 1000;
